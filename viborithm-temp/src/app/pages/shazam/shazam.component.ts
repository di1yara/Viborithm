...
export class ShazamComponent {
  ...
  constructor(
    private shazam: ShazamService,
    private ngZone: NgZone,
    private cdr: ChangeDetectorRef
  ) {
    afterNextRender(() => {
      console.log('Shazam component ready');
    });
  }
  ...
  identifySong(audioBlob: Blob) {
    ...
    this.timeoutHandle = setTimeout(() => {
      ...
      if (this.state === 'loading') {
        this.state = 'error';
        this.errorMessage = 'Request timed out. Check your API key.';
        this.cdr.detectChanges();
      }
    }, 20000);
    ...
    this.shazam.recognizeSong(audioBlob).subscribe({
     next: (data) => {
      ...
      if (track) {
        ...
        genres: { primary: track.genres?.primary || '' }
    };
    this.state = 'result';
  } else {
    this.state = 'notfound';
  ...
  this.cdr.detectChanges();
      ...
      error: (err) => {
        ...
        clearInterval(this.loadingInterval);
        if (err.status === 403) {
          this.errorMessage = 'Invalid API key or not subscribed on RapidAPI.';
        } else if (err.status === 429) {
          this.errorMessage = 'Free plan limit reached.';
        } else if (err.status === 0) {
          this.errorMessage = 'Network/CORS error.';
        } else {
